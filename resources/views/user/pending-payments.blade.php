...
@section('title', 'پرداخت‌های در انتظار - CryptoPay')
@section('page-title', 'پرداخت‌های در انتظار')
@section('page-subtitle', 'درخواست‌های پرداختی که منتظر تایید شما هستند')
...
@if($payments && $payments->count() > 0)
        ...
        @foreach($payments as $payment)
            <div class="bg-white rounded-lg shadow p-6 border-l-4 border-yellow-500" data-payment-item>
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-800">{{ $payment->invoice_number ?? 'درخواست پرداخت' }}</h3>
                        <p class="text-sm text-gray-500">از: {{ $payment->merchant->name ?? 'نامشخص' }}</p>
                    </div>
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-yellow-100 text-yellow-800">
                        <i class="fas fa-hourglass-end ml-2"></i>منتظر پرداخت
                    </span>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4 mb-6 p-4 bg-gray-50 rounded-lg">
                    <div>
                        <p class="text-xs text-gray-500 mb-1">مبلغ</p>
                        <p class="font-bold text-gray-800">{{ number_format($payment->amount, 8) }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">ارز</p>
                ...
                <div class="flex gap-3">
                    <a href="{{ url('/pay/' . $payment->token) }}" class="flex-1 bg-indigo-600 text-white py-2 rounded-lg font-semibold hover:bg-indigo-700 transition text-center">
                        <i class="fas fa-check-circle ml-2"></i>پرداخت کنید
                    </a>
                    <form action="{{ route('payment-request.reject', $payment->id) }}" method="POST" class="px-6">
                        @csrf
                        <button type="submit" class="px-6 py-2 bg-gray-200 text-gray-800 rounded-lg font-semibold hover:bg-gray-300 transition" onclick="return confirm('آیا مطمئن هستید که می‌خواهید این درخواست را رد کنید؟')">
                            <i class="fas fa-times ml-2"></i>رد کنید
                        </button>
                    </form>
                </div>
            </div>
    ...
    <div class="mt-6 flex items-center justify-between">
        <p class="text-sm text-gray-600">نمایش {{ $payments->count() }} درخواست</p>
...
@else
    <div class="bg-white rounded-lg shadow p-12 text-center">
        ...
        <p class="text-gray-500 mb-4">هیچ درخواست پرداختی در انتظار شما نیست</p>
