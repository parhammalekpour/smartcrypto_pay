...
<body x-data="{ sidebarOpen: false, notificationOpen: false, notificationCount: 0, notifications: [] }" x-init="(()=>{ try { if (!window.NOTIFICATION_ENDPOINTS) { window.NOTIFICATION_ENDPOINTS = {
        ...
        const syncNotificationsState = (listData, count = null) => {
            ...
            component.notifications = items;
            if (count === null) {
                component.notificationCount = items.filter(n => !n.read).length;
            } else {
                component.notificationCount = Number(count || 0);
            }
        };
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (newItems.length > 0) {
                                const merged = newItems.concat(component.notifications || []);
                                component.notifications = merged.slice(0, 50);
                                component.notificationCount = component.notifications.filter(n => !n.read).length;
                                try { if (typeof window.showToast === 'function') { newItems.forEach(i=>window.showToast(i.title||'Notification', i.message||'')); } } catch(e){}
                            } else {
                                syncNotificationsState(listData, newCount);
                            }
                        } catch(e) {
                            syncNotificationsState(listData, newCount);
                        }
        ...
        fetch(window.NOTIFICATION_ENDPOINTS.list, {credentials: 'same-origin'})
            .then(r => r.json())
            .then(data => {
                syncNotificationsState(data, null);
            })
            .catch(() => {});
